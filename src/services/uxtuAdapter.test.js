...
describe("clampParam", () => {
  ...
  it("每个已知参数的钳位结果都落在自身区间内", () => {
    for (const [key, { min, max }] of Object.entries(PARAM_RANGES)) {
      expect(clampParam(key, min - 1000)).toBe(min);
      expect(clampParam(key, max + 1000)).toBe(max);
    }
  });
});
...
describe("HAL 枚举映射", () => {
  it("四个模式都有对应的 thermal_mode 值且互不重复", () => {
    const values = MODES.map((m) => thermalModeMap[m]);
