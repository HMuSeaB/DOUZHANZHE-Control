...
- **被拒的调速请求仍会写硬件**: `POST /api/fan/set-target` 旧实现先 `ApplyFanSpeed` 下发、后 `SavePerfOverrides` 校验。传入未知配置 id 时接口虽然返回 400，但硬件已经被写过了 —— 而且用的是 `FanRpmRange(null)` 的兜底区间 (0,4400)，**把模式钳位整个绕过去**。现改为先校验落盘、再下发硬件，并且下发复用钳位后的值，保证"硬件值 == 落盘值"（原先两处各自钳一次，存在漂移风险）。错误文案补上"也未下发硬件"，让前端能区分两种失败语义
- **CPU 温度缺上界校验**: `HardwareAbstractionLayer.CpuTemperature` 旧代码只判 `>0 && <128` 就返回，于是 100~127°C 的 EC 脏读会被当成合法摄氏度直接放行（与 25249 RPM 脏读同源的风险）。风扇路径早就有 `ReadValidatedFanRpm` 做物理上限守卫，温度路径没有。现对齐风扇路径：新增 115°C 合理上界、脏读丢弃并重试 3 次、超限日志带 30s 节流（250ms 轮询下不节流会刷爆日志）

### 📋 说明

- 两处缺陷都是在 fix.6 真机验证过程中发现的，均属"错误路径也能改硬件"这一类
- 验证方式：3101 开发实例 + `regression-api.js`（`[3b]` 检测项由 WARN 转为通过）；另用 `dotnet Douzhanzhe.API.dll` 以普通权限起实例，可跑通 HTTP 层与 `[FanEC]` 日志
...
- **风扇偏离告警根因处理**: 24h 实机日志统计证实 — 斗战档(ITSM=3)固件全权接管风扇,手动写入 163 样本 0 跟随;大风扇物理上限 ~3000 RPM(CPU 88°C 时 EC 自动模式也仅 2921)。曲线高温段目标(大扇 4000+)必然路由进斗战档且物理不可达,导致持续偏离告警
- **目标钳位**: 曲线目标按实机物理上限钳位(大扇 3000 / 小扇 7200 RPM),路由随之落在均衡/野兽档(手动可控区);钳位状态变化时记日志,`route-info` 新增 `curveClamped` 字段。降温后钳位自动解除
- **前端提示**: 风扇控制页说明文案同步实机校准结论(斗战档写入无效 + 物理上限)

### 📋 说明

- 散热能力无损:斗战档 EC 自动转速与大扇物理上限相同(~2920),钳位到野兽/均衡档手动控制不损失风量
- ITSM 防抢占重写(memory-fix.2)保留,均衡/野兽档下仍需对抗固件改写 0xE4
...
## [2.0.1-memory-fix.7] — 2026-08-06

v2.0 预发布版本：全新界面、架构迁移与稳定修复

> 本版本为 v2.0 首个预发布版本，包含自 1.6.10 以来的全部改进。
> 由于本次涉及底层架构迁移，为稳妥起见暂不自动推送，老用户可手动下载体验。

### 🎉 全新体验

- **全新界面**: 采用 Fluent 设计语言，融入毛玻璃质感，支持深浅色主题与自定义配色
- **Intel 机型支持（本次重点）**: 此前版本不支持 Intel 机型，本次新增 CPU 功耗控制（PL1/PL2）
- **平台控制**: 键盘灯、GPU 模式、FN 锁、大写/数字小键盘锁、触控板锁及 EC 传感器信息
- **系统信息**: 硬件配置卡片网格，支持手动刷新

### 🎮 游戏与快捷键

- **游戏自动切换**: 原有功能保留，重新设计游戏卡片墙，可与自定义配置搭配使用；支持 Steam / Epic 扫描导入
- **快捷键**: 原有功能保留；修复录制不生效、性能模式快捷键切换不触发的问题

### 🌀 风扇与散热

- **自定义风扇曲线**: 原有功能保留，支持预设模板与拖动控制点
- **手动调速**: 按模式限制安全转速范围，与曲线互斥
- **风扇偏离守护**: 转速偏离目标时自动检测与恢复

### 🛠 可靠性与数据

- **底层驱动架构迁移（PawnIO）**: 替换旧版硬件访问方案，提升系统稳定性，降低游戏反作弊冲突与蓝屏风险
- **配置体系升级**: 支持完全自定义的用户配置，可复制、可无限新增；非斗战者机型会自动隐藏不支持的性能模式切换与 EC 控制
- **备份与恢复**: 新增按分类导出/导入，跨机型导入将整份拒绝，避免错误配置
- **自定义壁纸**: 原有功能保留，新增模糊度调节
- **应用内更新**: 原有功能保留，正式版发布后将自动提示更新

### 🔧 修复

- **设置页**: 修复打开设置页崩溃的问题
- **风扇曲线**: 修复停止曲线后自定义参数被清空的问题
- **平台控制**: 修复 EC 传感器信息不显示的问题
- **配置持久化**: 修复 AMD 机型功耗/温度设置保存后重启丢失、配置写入异常导致文件损坏等问题
- **自定义壁纸**: 修复上传超限可能丢失现有壁纸的问题
- **游戏规则**: 修复操作失败时缺少错误提示的问题

### 📋 注意事项

- 老版本既有功能（自定义风扇曲线、游戏自动切换、快捷键、应用内更新、自定义壁纸等）均已迁移保留
- 预发布版本可能存在不稳定，建议先在备用环境测试
- 一键降压、多显示器 / DPI 专项优化将在后续版本提供

## [2.0.1-memory-fix.7] — 2026-06-22
...
- **GPU 频率偏移不生效**: 部分机型（如 RTX 5060 Laptop GPU）通过 KaronOC.dll 下发超频偏移无效，核心/显存频率无法调整。现在改用原生 NVAPI 直接 P/Invoke `nvapi64.dll`，采用与 UXTU/NvAPIWrapper 相同的最小化 V1 结构体方式下发 P-State 偏移，完全移除 KaronOC.dll 依赖
  > 技术细节: 使用 `NV_GPU_PERF_PSTATES20` V1/7316 结构体，`state_count=1, clock_count=2`，从零构造后直接调用 `GPU_SetPStates20`，不再先 `Get` 再 `Set`

### 新增

- **遥测快照日志**: ParameterGuard 每轮参数下发后 10 秒自动记录一条运行状态快照，包含 CPU 频率/温度、GPU 频率/显存/温度/功耗、风扇转速、NVAPI 偏移验证等完整数据。每 60 秒一条，方便回溯频率是否真正生效、是否被温度墙或功耗墙限制
- **超频偏移回读验证**: NVAPI 下发 P-State 偏移后自动回读驱动实际接受的值和当前运行频率，日志中同时显示请求值和实际值，可立即发现偏移是否被正确应用

### 移除

- **KaronOC.dll 依赖**: GPU 超频功能不再依赖第三方 KaronOC.dll，改为直接调用 `nvapi64.dll` 原生接口。减少外部依赖、提高兼容性

## [2.0.1-memory-fix.7] — 2026-06-21

各模式独立配置持久化 + 配置稳定性修复 + 混合模式外接显示器修复 + 风扇守护 + Shell 崩溃自动恢复

### 新增

- **各模式独立配置持久化**: 每个性能模式（安静/均衡/野兽/斗战）的自定义参数现在独立保存到后端配置文件，不再依赖浏览器 localStorage。重装系统、清除浏览器数据不会丢失设置，每个模式的功耗、频率、风扇、超频等参数各自记忆
  > ⚠️ 切换模式时界面会暂时锁定，需等待参数下发完成（约 3-5 秒）
...
- **手动风扇转速不稳定**: 手动设置的风扇转速容易被 EC 固件覆盖。现在改用与自定义风扇曲线相同的写入方式（WMI + EC 寄存器直写），并增加 ParameterGuard 每 60 秒守护
  > ⚠️ EC 会根据当前温度墙和功耗设置自动调节风扇，实际转速可能低于设定值。例如斗战模式下需要将温度墙设为 95°C、长时功耗 90W、短时功耗 120W 并开启 CPU 压力测试，风扇才能达到目标转速 4300 RPM。低温度墙或低功耗设置下，EC 会主动限制风扇转速
...
- **游戏扫描无结果时弹窗无法关闭**: 扫描 Steam/Epic 游戏未发现新游戏时，弹窗没有关闭按钮导致界面卡死。现在无结果时显示关闭按钮
- **系统配置信息无法刷新**: BIOS 版本、电池容量等硬件信息只显示首次加载的缓存，升级 BIOS 或更换硬件后不会更新。现在"电脑配置"卡片右上角新增刷新按钮，支持手动拉取最新数据

### 改进

- **模式切换更可靠**: 切换模式时所有参数通过后端原子化完成（写入配置 → 切换硬件 → 返回确认），切换期间界面会短暂锁定防止误操作。相比之前版本切换可能稍慢，但确保每个模式的设置完整保存且不会互相干扰
- **GPU 面板智能禁用**: 混合模式下自动禁用核心频率/显存频率调节（这些在混合模式下无效），集显模式下禁用所有 GPU 控件
- **风扇曲线停止后自动恢复参数**: 关闭自定义风扇曲线后，之前被曲线覆盖的功耗、频率等参数会自动恢复

## [2.0.1-memory-fix.7] — 2026-06-18
